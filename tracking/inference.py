...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # Define observation domains
    maxManhattanDist = (X_RANGE - 1) + (Y_RANGE - 1)
    maxObservedDist = maxManhattanDist + MAX_NOISE
    observationDomain = list(range(maxObservedDist + 1))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # 2. Iterate through elimination order
        for eliminationVariable in eliminationOrder:
            # 3a. Join factors containing the elimination variable
            factorsNotJoined, joinedFactor = joinFactorsByVariable(currentFactorsList, eliminationVariable)

            # Check if any factors were joined
            if joinedFactor is None:
                 currentFactorsList = factorsNotJoined
                 continue # No factors contained this variable

            # 3b. Handle special case: factor with only one unconditioned variable
            if len(joinedFactor.unconditionedVariables()) == 1:
                # Discard the factor as eliminating the only unconditioned variable results in a factor of 1
                currentFactorsList = factorsNotJoined
            else:
                # 3c. Eliminate the variable
                eliminatedFactor = eliminate(joinedFactor, eliminationVariable)
                # 3d. Update factor list
                currentFactorsList = factorsNotJoined + [eliminatedFactor]

        # 4. Join remaining factors
        if not currentFactorsList:
             # Handle case where all factors might have been eliminated (e.g., query is independent of evidence)
             # Need to construct a factor representing uniform probability over query variables?
             # For now, assume this won't happen or needs specific handling based on expected behavior.
             # Let's try joining an empty list, which should be handled by joinFactors (returns None or empty factor)
             finalJointFactor = joinFactors([]) # Or handle appropriately
             # If joinFactors returns None, we might need to create a factor over query vars
             # For now, let's assume joinFactors handles empty list gracefully or this case isn't hit by tests.
        else:
            finalJointFactor = joinFactors(currentFactorsList)


        # 5. Normalize
        # Check if finalJointFactor is None before normalizing
        if finalJointFactor is None:
             # This case needs clarification. What should be returned if no factors remain?
             # Maybe a uniform distribution over query variables?
             # Let's return None for now, assuming tests cover valid scenarios.
             return None
        
        resultFactor = normalize(finalJointFactor)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        total = self.total()
        if total == 0:
             # Or handle appropriately, maybe raise error if sampling from zero total is invalid
             return None 
        
        # Ensure items are processed in a consistent order for reproducibility if needed,
        # although random sampling inherently introduces variability. Sorting is not strictly
        # necessary for correctness but can help in debugging or specific scenarios.
        items = sorted(self.items()) 
        
        rand_val = random.uniform(0, total)
        upto = 0
        for key, value in items:
            if upto + value >= rand_val:
                return key
            upto += value
        # Should not be reached if total > 0 and items is not empty
        # If self is empty, total would be 0, handled above.
        # If total > 0, the loop must return a key.
        # Add a fallback just in case, though it indicates an issue.
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            ...
            if noisyDistance is None:
                # Observation is None, but ghost is not in jail
                return 0.0
            else:
                # Ghost is not in jail and observation is a number
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            ...
            unnormalizedPosterior = observationProb * priorBelief
            
            newBeliefs[pos] = unnormalizedPosterior
        ...
        for oldPos in self.allPositions:
            ...
            if priorBelief > 0:
                ...
                for newPos, transitionProb in newPosDist.items():
                    ...
                    newBeliefs[newPos] += priorBelief * transitionProb
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            position_index = i % num_legal_positions
            particle_position = self.legalPositions[position_index]
            self.particles.append(particle_position)

    def getBeliefDistribution(self):
       
        belief_distribution = Counter() 
        for particle_position in self.particles:
            belief_distribution[particle_position] += 1
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particlePos in self.particles:
            weight = self.getObservationProb(observation, pacmanPosition, particlePos, jailPosition)
            weightDistribution[particlePos] += weight # Add weight (particles at same pos accumulate weight)
        ...
        for _ in range(self.numParticles):
            sampledPos = weightDistribution.sample()
